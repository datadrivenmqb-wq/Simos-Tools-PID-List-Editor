...
How to use:
1. Select a base logging list to start with. This can be what you're using currently, or something that mostly fits your needs. All PIDs from this initial base list will be "activated" or checked by default. Green PIDs are native PIDs direct from the car, blue PIDs are calculated (based on having a 0xffff or 0xffffffff address, depending on whether it is a Mode 22 or HSL list). Note that the editor expects any CSVs to be in the standard Simos Tools format: "Name", "Unit", "Equation", "Format", "Address", "Length", "Signed", "ProgMin", "ProgMax", "WarnMin", "WarnMax", "Smoothing", "Enabled", "Tabs", "Assign To". If an additional "Description" (or similar) column is present after "Assign To", that will also be imported to the "Description" column for notation/reference purposes. 
